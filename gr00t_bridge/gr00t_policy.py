...
#
# GR00T Inference Wrapper for ROS2 integration
#
# This module provides a clean wrapper around the Isaac-GR00T Gr00tPolicy
# for use in ROS2-based robotics applications.
#
# Design Goals:
# 1. Support stable single-camera (ego_view) inference first
# 2. Flexible architecture for future multi-camera extension
# 3. Reuse the same transforms as training
# 4. Clear interface for ROS2 integration
...
class Gr00tInferenceWrapper:
    """Wrapper for Isaac-GR00T model inference in ROS2 environments.
    
    This wrapper:
    1. Loads the GR00T model from a checkpoint
    2. Configures the data transforms to match training
    3. Provides a clean interface for inference
    ...
    Example usage:
        ...
        action = wrapper.predict(
            images={"ego_view": head_camera_image},  # (H, W, C) uint8
            state={
    ...
    def __init__(
        ...
        if data_config is None:
            ...
            self.data_config = FFWSG2InferenceConfig()
        else:
        ...
        print(f"[Gr00tInferenceWrapper] Model loaded successfully!")
    ...
    def predict(
        ...
        Args:
            images: Dict mapping camera names to images (H, W, C) uint8
                    Example: {"ego_view": head_camera_image}
            state: Dict mapping state component names to joint positions
    ...
    def video_keys(self) -> List[str]:
        ...
        return self.data_config.video_keys
...
def test_inference():
    ...
    print("=" * 60)
    print("GR00T Inference Wrapper Test")
    print("=" * 60)
    ...
    if not Path(checkpoint_path).exists():
        ...
        print("       This test requires a trained model checkpoint.")
        return False
    ...
    # Create dummy inputs
    dummy_image = np.random.randint(0, 255, (376, 672, 3), dtype=np.uint8)
    dummy_state = {
        "left_arm": np.zeros(7),
