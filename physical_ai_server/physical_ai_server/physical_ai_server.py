...
class PhysicalAIServer(Node):
    ...
    def __init__(self):
        ...
        # Declare runtime adjustable parameters for inference
        self._init_inference_parameters()
    ...
    def _init_inference_parameters(self):
        """Initialize runtime adjustable parameters for inference control."""
        self.declare_parameter('inference.velocity_scale', 1.0)
        self.declare_parameter('inference.trajectory_duration', 0.1)
        self.get_logger().info(
            f'Inference parameters initialized: '
            f'velocity_scale={self.get_parameter("inference.velocity_scale").value}, '
            f'trajectory_duration={self.get_parameter("inference.trajectory_duration").value}'
        )
    ...
    def _inference_timer_callback(self):
        ...
        try:
            ...
            # Get runtime adjustable parameters
            velocity_scale = self.get_parameter('inference.velocity_scale').value
            trajectory_duration = self.get_parameter('inference.trajectory_duration').value

            self.get_logger().info(
                f'Action data: {action}, velocity_scale: {velocity_scale}')
